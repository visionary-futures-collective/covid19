---
layout: page
title: Advocacy
permalink: /advocacy
---
# making a box

<div class="row">
  <div class="column">
	<div class="container">
	  <img src="img/caregiver-whofor.png" alt="Avatar" class="image" style="width:100%">
	  <div class="middle">
	    <div class="text"> I feel totally abandoned by the university and like they will just want me to quit, which I can’t afford.</div>
	  </div>
	</div>
  </div>
  <div class="column">
	<div class="container">
	  <img src="img/caregiver-whofor.png" alt="Avatar" class="image" style="width:100%">
	  <div class="middle">
	    <div class="text">There is absolutely no support whatsoever for caregivers that do not fit normative ideals of what families look like. We are struggling.</div>
	  </div>
	</div>
  </div>
  <div class="column">
	<div class="container">
	  <img src="img/caregiver-whofor.png" alt="Avatar" class="image" style="width:100%">
	  <div class="middle">
	    <div class="text">Caregiving knows no age boundaries, and can be needed at any time. American policies on family care ... almost entirely ignore care for ADULTS as a responsibility.</div>
	  </div>
	</div>
  </div>
</div>



<h2>Opacity with Box</h2>
<p>Hover over the image to see the effect.</p>

<div class="container">
  <img src="img/caregiver-whofor.png" alt="Avatar" class="image" style="width:100%">
  <div class="middle">
    <div class="text">John Doe</div>
  </div>
</div>
